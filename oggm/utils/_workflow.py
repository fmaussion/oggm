...
import oggm.utils.serialize as serialize
...
def get_centerline_lonlat(gdir,
    ...
    if flowlines_output or geometrical_widths_output or corrected_widths_output:
        cls = gdir.read_npz('inversion_flowlines')
    else:
        cls = gdir.read_npz('centerlines')
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            if gdir.has_file('inversion_output'):
                ...
                vol_bsl = []
                vol_bwl = []
                cl = gdir.read_npz('inversion_output')
                for c in cl:
                    vol.extend(c['volume'])
...
def idealized_gdir(surface_h, widths_m, map_dx, flowline_dx=1,
    ...
    gdir.write_npz([fl], 'inversion_flowlines')
...
class GlacierDirectory(object):
    ...
    def has_file(self, filename, filesuffix='', _deprecation_check=True):
        ...
        if os.path.exists(fp):
            return True
        # Directories written before the npz switch still hold pickles
    ...
    def read_pickle(self, filename, use_compression=None, filesuffix=''):
        ...
        fp = self.get_filepath(filename, filesuffix=filesuffix)
        if fp.endswith('.npz'):
            # These basenames moved to npz; a pickle is the legacy layout
            fp = fp[:-4] + '.pkl'
    ...
    def read_npz(self, filename, filesuffix=''):
        """Reads a data file located in the directory.

        The file holds plain numpy arrays plus a short description of how to
        rebuild the original OGGM objects from them (see
        :py:mod:`oggm.utils.serialize`). Nothing stored in it refers to an
        OGGM or a shapely class, so it stays readable across versions of
        either -- which is the point of the format.
        ...
        Returns
        -------
        The reconstructed object: a list of flowlines, a dict, ...
        """
        fp = self.get_filepath(filename, filesuffix=filesuffix)

        if not os.path.exists(fp):
            # Legacy layout: directories built before the npz switch store
        ...
        return serialize.decode(filename, arrays, meta, gdir=self)
    ...
    def write_npz(self, var, filename, filesuffix=''):
        """Writes a variable to disk as plain numpy arrays.

        The object is encoded by :py:mod:`oggm.utils.serialize`, so the
        resulting file contains arrays only -- no OGGM or shapely classes.
    ...
    def get_inversion_flowline_hw(self):
        ...
        h = np.array([])
        w = np.array([])
        fls = self.read_npz('inversion_flowlines')
        for fl in fls:
...
def initialize_merged_gdir(main, tribs=[], glcdf=None,
    ...
    mfls = main.read_npz('model_flowlines')
    ...
    for trib in tribs:
        geom = trib.read_npz('geometries')['polygon_hr']
    ...
    merged.write_npz(mfls, 'model_flowlines')
